...
/**
 * @description: 直播token拦截器
 * @author: danmo
 * @create: 2021-09-27 22:36
 **/
@Slf4j
@Component
public class WeLiveAccessTokenInterceptor extends WeForestInterceptor implements Interceptor<WeResultVo> {
    ...
    public boolean beforeExecute(ForestRequest request) {
        ...
        String token = iQwAccessTokenService.findLiveAccessToken(getCorpId(request));
    ...
    public void onRetry(ForestRequest request, ForestResponse response) {
        ...
        if (!ObjectUtil.equal(WeErrorCodeEnum.ERROR_CODE_OWE_1.getErrorCode(), weResultVo.getErrCode())
            ...
            iQwAccessTokenService.removeLiveAccessToken(getCorpId(request));
            String token = iQwAccessTokenService.findLiveAccessToken(corpId);
            request.replaceOrAddQuery("access_token", token);
        }
    }
}
