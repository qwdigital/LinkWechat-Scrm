 ...
 * @author danmo
 */
public class WeMsgQiRuleThreadExecutor {
    ...
    private WeMsgQiRuleThreadExecutor() {
    }
    ...
    public static ThreadPoolExecutor getInstance() {
        if (executor == null) {
            synchronized (WeMsgQiRuleThreadExecutor.class) {
                if (executor == null) {
                    executor = new ThreadPoolExecutor(5,// 核心线程数
                            ...
                            new ThreadFactoryBuilder().setNameFormat("qi-rule-pool-%d").build(),
