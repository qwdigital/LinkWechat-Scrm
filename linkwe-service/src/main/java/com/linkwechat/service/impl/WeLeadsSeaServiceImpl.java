...
import com.linkwechat.mapper.WeLeadsSeaRuleRecordMapper;
...
public class WeLeadsSeaServiceImpl extends ServiceImpl<WeLeadsSeaMapper, WeLeadsSea> implements IWeLeadsSeaService {
    ...
    @Override
    public Map<String, Object> getAllDataAnalysis() {
        Map<String, Object> result = new HashMap<>(8);
        //1、有效线索总数
        int effectiveLeadsCount = weLeadsService.count(Wrappers.lambdaQuery(WeLeads.class).eq(WeLeads::getDelFlag, Constants.COMMON_STATE));
        result.put("effectiveLeadsCount", effectiveLeadsCount);
        //2、加V客户数
        int addVCustomersCount = weLeadsService.count(Wrappers.lambdaQuery(WeLeads.class)
                .eq(WeLeads::getWeixinStatus, WeiXinStatusEnum.ADD_WECHAT_APPROVED.getCode())
                .eq(WeLeads::getDelFlag, Constants.COMMON_STATE));
        result.put("addVCustomersCount", addVCustomersCount);
        //3、线索上门率
        int leadsVisitRate1 = weLeadsService.count(Wrappers.lambdaQuery(WeLeads.class)
                .eq(WeLeads::getLeadsStatus, LeadsStatusEnum.VISIT.getCode())
                .eq(WeLeads::getDelFlag, Constants.COMMON_STATE));


        //4、线索退回率
        int leadsReturnRate1 = weLeadsService.count(Wrappers.lambdaQuery(WeLeads.class)
                .eq(WeLeads::getLeadsStatus, LeadsStatusEnum.RETURNED.getCode())
                .eq(WeLeads::getDelFlag, Constants.COMMON_STATE));
        if (0 != effectiveLeadsCount) {
            double leadsVisitRate = NumberUtil.div(leadsVisitRate1, effectiveLeadsCount);
            double leadsReturnRate = NumberUtil.div(leadsReturnRate1, effectiveLeadsCount);
            result.put("leadsVisitRate", leadsVisitRate);
            result.put("leadsReturnRate", leadsReturnRate);
        }

        //5、线索日跟进人数
//        QueryWrapper<WeLeadsFollowRecord> queryWrapper = new QueryWrapper<>();
//        queryWrapper.eq("del_flag", Constants.COMMON_STATE)
//                .isNotNull("record_content")
//                .eq("DATE_FORMAT(create_time, '%Y-%m-%d')", DateUtil.today());
//        List<WeLeadsFollowRecord> weLeadsLeadsFollowRecords = weLeadsFollowRecordMapper.selectList(queryWrapper);
//        int leadsDailyFollowUpCount = weLeadsLeadsFollowRecords.stream().distinct().map(WeLeadsFollowRecord::getWeLeadsLeadsId).collect(Collectors.toList()).size();
//        result.put("leadsDailyFollowUpCount", leadsDailyFollowUpCount);
        return result;
    }
    ...
    public Map<String, Object> getCounts(Long seaId) {
        ...
        // 1、有效线索总量
        int leadsNum = weLeadsService.count(Wrappers.lambdaQuery(WeLeads.class).eq(WeLeads::getSeaId, seaId)
//                .eq(WeLeadsLeads::getLeadsStatus, LeadsStatusEnum.BE_FOLLOWING_UP.getCode())
                .eq(WeLeads::getDelFlag, Constants.COMMON_STATE));
        result.put("leadsNum", leadsNum);

        // 2、总跟进量
        int allFollowNum = weLeadsService.count(Wrappers.lambdaQuery(WeLeads.class).eq(WeLeads::getSeaId, seaId)
                .eq(WeLeads::getLeadsStatus, LeadsStatusEnum.BE_FOLLOWING_UP.getCode())
                .eq(WeLeads::getDelFlag, Constants.COMMON_STATE));
        result.put("allFollowNum", allFollowNum);

//        QueryWrapper<WeLeadsLeadsFollowRecord> queryWrapper = new QueryWrapper<>();
//        queryWrapper.in("sea_id", seaId);
//        queryWrapper.eq("DATE_FORMAT(create_time, '%Y-%m-%d')", DateUtil.today());
//        int todayReceiveNum = weLeadsLeadsFollowRecordService.count(queryWrapper);
//        result.put("todayReceiveNum", todayReceiveNum);
//
//        // 4、较昨日领取量  DateUtil.yesterday().toDateStr()
//        QueryWrapper<WeLeadsLeadsFollowRecord> query = Wrappers.query();
//        query.in("sea_id", seaId).eq("DATE_FORMAT(create_time, '%Y-%m-%d')", DateUtil.yesterday().toDateStr());
//
//        int yesterdayReceiveNum1 = weLeadsLeadsFollowRecordService.count(query);
...
//
//        // 5、今日有效跟进
//        QueryWrapper<WeLeadsLeadsFollowRecord> query1 = Wrappers.query();
//        query1.in("sea_id", seaId)
//                .isNotNull("record_content").eq("DATE_FORMAT(create_time, '%Y-%m-%d')", DateUtil.today());
//        int todayFollowNum = weLeadsLeadsFollowRecordService.count(query1);
//        result.put("todayFollowNum", todayFollowNum);
//
//        // 6、较昨日跟进
//        QueryWrapper<WeLeadsLeadsFollowRecord> query2 = Wrappers.query();
//        query2.in("sea_id", seaId)
//                .isNotNull("record_content")
//                .eq("DATE_FORMAT(create_time, '%Y-%m-%d')", DateUtil.yesterday().toDateStr());
//        int yesterdayFollowNum1 = weLeadsLeadsFollowRecordService.count(query2);
...
//        result.put("yesterdayFollowRatio", yesterdayFollowRatio);
     ...
     */
    @Override
    public List<SeaLeadsTrendVo> seaLeadsTrend(String seaId, String beginTime, String endTime) {
        if (StringUtils.isBlank(seaId)) {
            throw new ServiceException("公海池Id不能为null", HttpStatus.BAD_REQUEST);
        }
        log.info("-------------数据趋势 入参seaId:{},beginTime:{},endTime:{}", seaId, beginTime, endTime);
//        List<SeaLeadsTrendVo> seaLeadsTrendVos = weLeadsLeadsFollowRecordMapper.seaLeadsTrend(Long.valueOf(seaId), beginTime, endTime);
//        return seaLeadsTrendVos;
        return null;
    }

    @Override
    public List<WeLeadsSeaEmployeeRankVo> getCustomerRank(String seaId, String beginTime, String endTime) {
        if (StringUtils.isBlank(seaId)) {
            throw new ServiceException("公海池Id不能为null", HttpStatus.BAD_REQUEST);
        }
        log.info("-------------跟进员工 TOP5 入参seaId:{},beginTime:{},endTime:{}", seaId, beginTime, endTime);
//        List<WeLeadsSeaEmployeeRankVo> customerRank = weLeadsLeadsFollowRecordMapper.getCustomerRank(Long.valueOf(seaId), beginTime, endTime);
//        return customerRank;
        return null;
    }

    @Override
    public List<WeLeadsSeaDataDetailVo> getSeaDataDetail(String seaId, String beginTime, String endTime) {
        if (StringUtils.isBlank(seaId)) {
            throw new ServiceException("公海池Id不能为null", HttpStatus.BAD_REQUEST);
        }
        log.info("-------------数据明细 入参seaId:{},beginTime:{},endTime:{}", seaId, beginTime, endTime);
//        List<WeLeadsSeaDataDetailVo> seaDataDetail = weLeadsMapper.getSeaDataDetail(beginTime, endTime);
//        for (WeLeadsSeaDataDetailVo vo : seaDataDetail) {
//            // 1、总线索量
//            QueryWrapper<WeLeads> queryWrapper = new QueryWrapper<>();
//            queryWrapper.eq("sea_id", seaId)
//                    .eq("del_flag", Constants.COMMON_STATE)
//                    .le("DATE_FORMAT(create_time, '%Y-%m-%d')", vo.getDateTime());
//            int totalNum = weLeadsService.count(queryWrapper);
//            log.info("-------------日期: {},总线索量:{}", vo.getDateTime(), totalNum);
//
//            // 2、总跟进量
...
//                    .eq("del_flag", Constants.COMMON_STATE)
//                    .le("DATE_FORMAT(create_time, '%Y-%m-%d')", vo.getDateTime());
//            int allFollowNum = weLeadsService.count(allFollowQuery);
//            log.info("-------------日期: {},总跟进量:{}", vo.getDateTime(), allFollowNum);
//
//            // 3、今日领取量
//
//            QueryWrapper<WeLeadsLeadsFollowRecord> followRecordWrapper = new QueryWrapper<>();
//            followRecordWrapper.in("sea_id", seaId);
//            followRecordWrapper.eq("DATE_FORMAT(create_time, '%Y-%m-%d')", vo.getDateTime());
//            int todayReceiveNum = weLeadsLeadsFollowRecordService.count(followRecordWrapper);
//            log.info("-------------日期: {},今日领取量:{}", vo.getDateTime(), todayReceiveNum);
//
//            // 4、今日有效跟进
//            QueryWrapper<WeLeadsLeadsFollowRecord> query2 = new QueryWrapper<>();
//            query2.eq("sea_id", seaId)
//                    .isNotNull("record_content")
//                    .eq("DATE_FORMAT(create_time, '%Y-%m-%d')", vo.getDateTime());
//            int todayFollowNum = weLeadsLeadsFollowRecordService.count(query2);
//            log.info("-------------日期: {},今日有效跟进:{}", vo.getDateTime(), todayFollowNum);
//
...
//            }
//
//            log.info("-------------日期: {},今日跟进率:{}", vo.getDateTime(), todayFollowRatio);
//            vo.setTotalNum(totalNum);
//            vo.setAllFollowNum(allFollowNum);
//            vo.setTodayReceiveNum(todayReceiveNum);
//            vo.setTodayFollowNum(todayFollowNum);
//            vo.setTodayFollowRatio(todayFollowRatio);
//        }
//        return seaDataDetail;
        return null;
    }
