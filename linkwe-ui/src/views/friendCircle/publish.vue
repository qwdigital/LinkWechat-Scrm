          ...
          <div class="g-card g-pad20">
            ...
            <div class="my-cord">
              <el-form-item label="动态内容" required style="width: 50%; margin-bottom: 0" :error="welcomeMsgError">
                <el-input ref="msgTextarea" type="textarea" v-model="form.content" maxlength="2000" show-word-limit placeholder="请输入" :autosize="{ minRows: 5, maxRows: 20 }" clearable />
              </el-form-item>
            </div>
          </div>
    ...
    methods: {
      ...
      submit () {
        ...
        if (this.form.scopeType === 0) {
          this.form.customerTag = this.selectedTagList.map(dd => {
            return dd.tagId
          }).join(',')
          this.form.noAddUser = this.selectedUserList.map(dd => {
            return dd.userId
          }).join(',')
        }
