		...
		<div>
			<el-form :model="query" label-position="left" ref="queryForm" :inline="true" label-width="70px" class="top-search">
				...
				<el-form-item label="员工名" prop="addUserName">
					<el-input v-model="query.addUserName" placeholder="请输入员工名" clearable @keyup.enter.native="getList(1)" />
				</el-form-item>
		...
		<div>
			<el-table ref="myTable" v-loading="loading" :data="list" @selection-change="handleSelectionChange" max-height="600">
				...
				<el-table-column label="当前状态" align="center" prop="addState">
					<template slot-scope="{ row }">
						<el-tag v-if="row.addState === 0" size="mini" type="warning">
							待添加</el-tag>
				...
				<el-form ref="importForm" :model="form" :rules="formRules" label-position="right">
					...
					<el-form-item label="新客户标签" prop="customerSeasTags">
						<template v-for="(data, index) in form.customerSeasTags">
							<el-tag v-if="data.tagName" :key="index">
								{{data.tagName}}</el-tag>
						</template>
	...
	export default {
		...
		methods: {
			submitImport () {
				...
				let obj = {
					file: this.form.file,
					addUserId: this.form.weCustomerStaffs.map(dd => {
						return dd.staffId
					}).join(','),
					addUserName: this.form.weCustomerStaffs.map(dd => {
						return dd.staffName
					}).join(','),
					tagIds: this.form.customerSeasTags.length ? this.form.customerSeasTags.map(dd => {
						return dd.tagId
					}).join(',') : '',
					tagNames: this.form.customerSeasTags.length ? this.form.customerSeasTags.map(dd => {
						return dd.tagName
					}).join(',') : ''
				}
