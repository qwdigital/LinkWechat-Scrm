  ...
  <div class="wrap" v-loading="loading">
    <el-form :model="form" ref="form" :rules="rules" label-width="100px">
      ...
      <el-form-item label="执行群聊" prop="chatIdList">
        <el-tag size="medium" v-for="(group, index) in customerGroups" :key="index">{{ group.groupName }}
        </el-tag>
        <el-button type="primary" plain class="ml10" icon="el-icon-plus" size="mini" @click="dialogVisibleSelectCustomerGroup = true">{{ customerGroups.length ? '修改' : '添加' }}</el-button>
      </el-form-item>

      <el-form-item label="内容名称" prop="title">
        <el-input v-model="form.title" maxlength="220" show-word-limit :autosize="{ minRows: 5, maxRows: 20 }" placeholder="请输入" clearable />
      </el-form-item>
      ...
      <el-form-item label="消息内容" prop="content">
        <div class="content-left">
          ...
          <el-tabs v-model="activeName">
            <el-tab-pane name="0">
              <span slot="label">
                <i class="el-icon-date"></i> 文本 </span>
              <el-input v-model="form.content" type="textarea" maxlength="220" show-word-limit :autosize="{ minRows: 10, maxRows: 50 }" placeholder="请输入" class="mb8"></el-input>
            ...
            <el-tab-pane name="1">
              <span slot="label">
                <i class="el-icon-date"></i> 图片 </span>
    ...
    <!-- 选择素材弹窗 -->
    <!-- 文本 -->
    <SelectMaterial v-if="activeName === '0'" :key="0" :visible.sync="dialogVisibleSelectMaterial" type="4" @success="(data) => submitSelectMaterial(data, 4)">
    </SelectMaterial>

    <!-- 图片 -->
    <SelectMaterial v-else :key="1" :visible.sync="dialogVisibleSelectMaterial" type="0" @success="(data) => submitSelectMaterial(data, 0)">
    </SelectMaterial>

    <!-- 选择客户群聊 -->
    <SelectCustomerGroup :visible.sync="dialogVisibleSelectCustomerGroup" @success="submitSelectCustomerGroup" :multiSelect="true">
    </SelectCustomerGroup>
  </div>
</template>
